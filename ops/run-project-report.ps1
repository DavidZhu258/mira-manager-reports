...
function Get-Token {
    $tokenFile = [Environment]::GetEnvironmentVariable("MANAGER_REPORT_TOKEN_FILE")
    if (-not $tokenFile -or $tokenFile.Trim().Length -eq 0) {
        $tokenFile = Join-Path $env:USERPROFILE ".manager-report-token"
    }
    $managerReportApiKey = [Environment]::GetEnvironmentVariable("MANAGER_REPORT_API_KEY")
    if ($managerReportApiKey -and $managerReportApiKey.Trim().Length -gt 0) {
        return $managerReportApiKey.Trim()
    }
    ...
    throw "GPT token not found. Set MANAGER_REPORT_API_KEY, GPT55_PRO_API_KEY, or MANAGER_REPORT_TOKEN_FILE."
}
...
function New-FallbackManagerReport {
    ...
    $lines.Add("- 恢复 GPT token/API 调用环境后，重新运行报告生成以获得更完整的管理层摘要。")
